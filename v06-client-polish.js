...
nextFlowInfo = function(){
  ...
  x.actions=(x.actions||'')
    ...
    .replaceAll('Начать заново','Новый заказ');
  return x;
};
...
function scrubClientCopy(){
  ...
  for(const n of nodes){
    n.nodeValue=n.nodeValue
      ...
      .replace('marketplace показывает бизнес-статус','в личном кабинете отображается статус исполнения')
      .replace('Marketplace показывает бизнес-статус','В личном кабинете отображается статус исполнения')
      .replace('Начать заново','Новый заказ');
  }
}
